...
int main() {
  ...
  if (mmio(AON_CTRL_ADDR, AON_BACKUP15) == BACKUP15_MAGIC) {
    ...
    mmio(GPIO_CTRL_ADDR, GPIO_REG_IOF_EN) |= ~GREEN_LED;
    ...
    mmio(GPIO_CTRL_ADDR, GPIO_REG_IOF_EN) |= ~RED_LED;
    ...
    // clint
    uint32_t mtime_lo = mmio(CLINT_CTRL_ADDR, CLINT_MTIME);
    uint32_t mtime_hi = mmio(CLINT_CTRL_ADDR, CLINT_MTIME + 4);
    uint32_t next_mtime_lo = mtime_lo + 0x4000;
    // overflow, add 1 to mtime_hi
    uint32_t next_mtime_hi = mtime_hi + ((next_mtime_lo < mtime_lo) ? 1 : 0);

    mtime_lo = mmio(CLINT_CTRL_ADDR, CLINT_MTIME);
    mtime_hi = mmio(CLINT_CTRL_ADDR, CLINT_MTIME + 4);
    // 200002cc
    if (mtime_hi >= next_mtime_hi) {
      // 2000048c
    } else {
      // 200002d0
      // 200002d4
